...
esp_err_t scd41_stop_periodic_measurement(scd41_t* s) {
    ...
    return scd41_write_cmd(s, SCD41_CMD_STOP_PERIODIC);
}

// Query if a fresh measurement is ready (bit15 of status word).
esp_err_t scd41_data_ready(scd41_t* s, bool* ready) {
    ...
    // Bit15 indicates whether a new sample is available.
    *ready = (status & 0x8000u) != 0;
    return ESP_OK;
}
